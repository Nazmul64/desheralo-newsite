...
.nb-field { border: 1px solid #e5e7eb; padding: 0; position: relative; }
.nb-field-label { font-size: 12px; font-weight: 500; color: #6b7280; padding: 8px 14px 4px; display: block; }
...
/* ── Dropdown fix ── */
.nb-field .nb-select-wrap {
    position: relative;
    padding: 0 14px 10px;
}
.nb-field .nb-select-wrap select {
    width: 100%;
...
.nb-field .nb-select-wrap select:focus { border-color: #2563eb; }

.nb-field textarea { min-height: 80px; }
.nb-field.is-invalid { border-color: #dc2626; }
.nb-field.is-invalid .nb-select-wrap select { border-color: #dc2626; }
.nb-invalid-msg { font-size: 11.5px; color: #dc2626; padding: 2px 14px 6px; }
...
.nb-img-preview { max-height: 160px; border-radius: 6px; border: 1px solid #e5e7eb; }
.nb-img-thumb   { max-height: 160px; border-radius: 6px; border: 1px solid #e5e7eb; }
...
<div class="nb-wrap">
    ...
    <div class="nb-form-card">
        <form method="POST" action="{{ route('newblog.update', $blog->id) }}" enctype="multipart/form-data" id="blogForm">
            @csrf
            @method('PUT')
            ...
            <div class="nb-row">
                ...
                <div class="nb-field @error('summary') is-invalid @enderror" style="border-left:none;">
                    ...
                    <textarea name="summary" placeholder="Enter Summary" rows="3">{{ old('summary', $blog->summary) }}</textarea>
            ...
            <div class="nb-row">
                <div class="nb-field @error('description') is-invalid @enderror" style="border-top:none;">
                    ...
                    <textarea name="description" placeholder="Enter description" rows="4">{{ old('description', $blog->description) }}</textarea>
                ...
                <div class="nb-field @error('newsblogcategory_id') is-invalid @enderror" style="border-top:none; border-left:none;">
                    ...
                    <div class="nb-select-wrap">
                        <select name="newsblogcategory_id" id="categorySelect">
                            <option value="">-- Select Category --</option>
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}"
                                    {{ old('newsblogcategory_id', $blog->newsblogcategory_id) == $cat->id ? 'selected' : '' }}>
                                    {{ $cat->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="nb-select-wrap">
                        <select name="newssubblogcategory_id" id="subcategorySelect">
                            ...
                            @foreach($subcategories as $sub)
                                <option value="{{ $sub->id }}"
                                    {{ old('newssubblogcategory_id', $blog->newssubblogcategory_id) == $sub->id ? 'selected' : '' }}>
                                    {{ $sub->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="nb-row">
                <div class="nb-field @error('tags') is-invalid @enderror" style="border-top:none;">
                    ...
                    <input type="text" name="tags"
                           value="{{ old('tags', $blog->tags) }}" placeholder="Enter Tags">
                ...
                <div class="nb-field @error('status') is-invalid @enderror" style="border-top:none; border-left:none;">
                    ...
                    <div class="nb-select-wrap">
                        <select name="status">
                            ...
                            <option value="0" {{ old('status', $blog->status ? '1' : '0') == '0' ? 'selected' : '' }}>Unpublished</option>
                        </select>
                    </div>
...
document.getElementById('categorySelect').addEventListener('change', function () {
    ...
    var oldSubId  = "{{ old('newssubblogcategory_id', $blog->newssubblogcategory_id) }}";
