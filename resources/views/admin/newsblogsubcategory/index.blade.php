...
@section('content')
<style>
.sc-wrap { padding: 24px; background: #f5f6fa; min-height: 100vh; }
...
.sc-topbar h5 { margin: 0; font-size: 15px; font-weight: 600; color: #222; }
...
.sc-btn-add:hover { background: #1d4ed8; color: #fff; }
...
.sc-search-wrap input::placeholder { color: #9ca3af; }
.sc-search-btn {
...
.sc-card { background: #fff; border-radius: 8px; box-shadow: 0 1px 4px rgba(0,0,0,.06); overflow: hidden; }

.sc-table { width: 100%; border-collapse: collapse; font-size: 13.5px; }
.sc-table thead tr { background: #f9fafb; border-bottom: 1px solid #e5e7eb; }
.sc-table thead th { padding: 11px 14px; font-weight: 600; color: #374151; text-align: left; white-space: nowrap; }
.sc-table tbody tr { border-bottom: 1px solid #f0f1f3; transition: background .12s; }
.sc-table tbody tr:last-child { border-bottom: none; }
.sc-table tbody tr:hover { background: #fafbfc; }
.sc-table td { padding: 10px 14px; color: #374151; vertical-align: middle; }

.sc-check { width: 15px; height: 15px; accent-color: #2563eb; cursor: pointer; }
...
.sc-trash-btn:hover { color: #dc2626; background: #fef2f2; }
.sc-trash-btn.show { display: inline-flex; }
...
.sc-action-btn:hover { background: #f3f4f6; color: #111; }

.sc-dropdown { min-width: 130px; font-size: 13px; }
.sc-dropdown .dropdown-item { display: flex; align-items: center; gap: 8px; padding: 8px 14px; color: #374151; cursor: pointer; }
.sc-dropdown .dropdown-item:hover { background: #f3f4f6; }
.sc-dropdown .dropdown-item.del { color: #dc2626; }
.sc-dropdown .dropdown-item.del:hover { background: #fef2f2; }

.sc-badge { display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; }
.sc-badge.active   { background: #dcfce7; color: #16a34a; }
.sc-badge.inactive { background: #f3f4f6; color: #6b7280; }

.sc-empty { text-align: center; padding: 48px 0; color: #9ca3af; font-size: 14px; }
.sc-empty svg { width: 48px; height: 48px; margin-bottom: 10px; display: block; margin-inline: auto; }
...
<div class="sc-wrap">
    ...
    <div class="sc-topbar">
        ...
        <a href="{{ route('newsblogsubcategory.create') }}" class="sc-btn-add">+ Add Sub-Category</a>
    </div>
    ...
    <div class="sc-card">
        ...
        <div style="padding:14px 16px; border-bottom:1px solid #f0f1f3;">
            <form method="GET" action="{{ route('newsblogsubcategory.index') }}" class="sc-controls" id="filterForm">

                <input type="checkbox" class="sc-check" id="checkAll">

                <button type="button" id="bulkDeleteBtn" class="sc-trash-btn" title="Delete selected">
                ...
                <select name="show" class="sc-show-select" onchange="this.form.submit()">
                ...
                <div class="sc-search-wrap">
                    ...
                    <button type="submit" class="sc-search-btn">
        ...
        <div style="overflow-x:auto;">
            <table class="sc-table">
                <thead>
                    <tr>
                        <th style="width:42px;"><input type="checkbox" class="sc-check" id="checkAllTable"></th>
                ...
                <tbody>
                    ...
                    <tr id="row-{{ $sub->id }}">
                        ...
                        <td><input type="checkbox" class="sc-check row-check" value="{{ $sub->id }}"></td>

                        <td style="color:#6b7280; text-align:center;">{{ $subcategories->firstItem() + $index }}</td>

                        <td style="font-weight:500; color:#1f2937;">{{ $sub->name }}</td>
                        ...
                        <td>
                            <span class="sc-badge {{ $sub->status ? 'active' : 'inactive' }}">
                        ...
                        <td>
                            <div class="dropdown">
                                <button class="sc-action-btn" data-bs-toggle="dropdown" aria-expanded="false">⋮</button>
                                <ul class="dropdown-menu dropdown-menu-end sc-dropdown shadow-sm">
                                    <li>
                                        <a class="dropdown-item" href="{{ route('newsblogsubcategory.edit', $sub->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
                                    ...
                                    <li>
                                        <button type="button"
                                            ...
                                            data-id="{{ $sub->id }}"
                                            data-url="{{ route('newsblogsubcategory.destroy', $sub->id) }}"
                                            data-title="{{ $sub->name }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24"
...
{{-- Bulk Delete URL --}}
<span id="sc-bulk-url" data-url="{{ route('newsblogsubcategory.bulkDestroy') }}" style="display:none;"></span>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (checkAll)      checkAll.addEventListener('change',      function () { syncAllCheckboxes(this.checked); });
    ...
    document.addEventListener('change', function (e) {
        ...
        var s       = all.length > 0 && all.length === checked.length;
        var ind     = checked.length > 0 && checked.length < all.length;
    ...
    if (bulkBtn) {
        bulkBtn.addEventListener('click', function () {
            ...
            var bulkUrl = document.getElementById('sc-bulk-url').getAttribute('data-url');
