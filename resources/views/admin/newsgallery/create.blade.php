...
@error('title')  .title-input  { border-color: #ef4444 !important; } @enderror
@error('photo')  .photo-input  { border-color: #ef4444 !important; } @enderror
...
<div class="container-fluid py-4">
    ...
    <div class="sc-page-header">
        <span class="page-title">Add Gallery Item</span>
    ...
    <div class="form-card">
        <form action="{{ route('newsgallery.store') }}" method="POST" enctype="multipart/form-data">
            @csrf

            {{-- Title --}}
            <div class="mb-4">
                <label class="fc-label">Title <span style="color:#ef4444;">*</span></label>
                <input type="text" name="title" class="fc-input title-input @error('title') is-invalid @enderror"
                       placeholder="Enter gallery title" value="{{ old('title') }}" autocomplete="off">
                @error('title')
            ...
            {{-- Photo --}}
            <div class="mb-4">
                <label class="fc-label">Photo <span style="color:#ef4444;">*</span></label>
                <input type="file" name="photo" id="photoInput"
                       class="fc-input photo-input @error('photo') is-invalid @enderror"
                       accept="image/*">
                @error('photo')
                    <div style="font-size:.78rem;color:#ef4444;margin-top:4px;">{{ $message }}</div>
                @enderror
